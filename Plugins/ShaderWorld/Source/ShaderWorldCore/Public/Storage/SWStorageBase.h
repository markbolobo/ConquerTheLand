...
#include "SWStorageBase.generated.h"

/**
 * 
 */
UCLASS(BlueprintType, AutoCollapseCategories = ("Import Settings"))
class SHADERWORLDCORE_API USWStorageBase : public UObject
{
	GENERATED_BODY()

private:
	/** Stores the path to the original source file. Empty if dynamically created. */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Import Settings", meta = (AllowPrivateAccess = "true"))
		FDirectoryPath DestinationPath;

public:

	UPROPERTY(Transient)
		bool bTriggerHeightmapExport = false;
...
public:
	...
	//FSW_PointerTree<TerrainStorage> StorageTree;
